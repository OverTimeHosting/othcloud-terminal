			...
			},
			{
				...
				item: {
					...
					command: {
						...
						title: localize({ key: 'miNewTerminalWithProfile', comment: ['&& denotes a mnemonic'] }, "New Terminal With &&Profile...")
					},
					order: 1.5
				}
			},
	...
	for (const menuId of [MenuId.EditorTitle, MenuId.CompactWindowEditorTitle]) {
		...
		MenuRegistry.appendMenuItem(menuId, {
			command: {
				...
				title: localize('workbench.action.terminal.splitEditorSide', "Split Terminal Right"),
				icon: Codicon.splitHorizontal
			},
			group: 'navigation',
			order: 100000,
